...
import { startCheckout, redeemCoupon } from "../lib/api";

const VALID_COUPONS = ["CPSI2026", "THIEF2026"];
...
export default function UpgradeModal({ onClose, userStatus, isAuthenticated, onRequestAuth, onPurchaseSuccess }) {
  ...
  const handleCoupon = async () => {
    if (!isAuthenticated) { onRequestAuth("coupon"); return; }
    ...
    try {
      ...
      setCouponError(null);
      if (result.benefit_type === "lifetime_pro" && onPurchaseSuccess) {
        setTimeout(() => onPurchaseSuccess(), 1500);
      }
    } catch (e) {
      setCouponError(e.message || "Couldn't apply code");
      setCouponSuccess(null);
    }
  };
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(10px)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "20px", overflowY: "auto" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#18181f", border: "1px solid rgba(212,168,67,0.25)", borderRadius: "20px", padding: "32px 28px", maxWidth: "480px", width: "100%", position: "relative", animation: "fadeUp 0.4s ease", marginTop: "20px", marginBottom: "20px" }}>
        ...
        <FeatureBlock title="Here's everything you unlock" features={[
          { emphasis: "Unlimited pattern searches", suffix: " across 25 unexpected domains" },
        ...
        {/* Coupon section */}
        {!showCoupon ? (
          <p style={{ fontFamily: "'Lato'", fontSize: "12px", color: "rgba(255,255,255,0.4)", textAlign: "center", lineHeight: 1.6 }}>
            Workshop attendee? <button onClick={() => setShowCoupon(true)} style={{ background: "none", border: "none", padding: 0, color: "#d4a843", fontSize: "12px", fontFamily: "'Lato'", cursor: "pointer", textDecoration: "underline", textDecorationColor: "rgba(212,168,67,0.4)" }}>Enter your code</button> to redeem your benefits.
          </p>
        ) : (
          <div>
            <p style={{ fontFamily: "'Lato'", fontSize: "12px", color: "rgba(255,255,255,0.5)", marginBottom: "8px" }}>{isAuthenticated ? "Enter your workshop code" : "Sign in to redeem your code"}</p>
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              <button onClick={handleCoupon} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "8px", padding: "10px 16px", color: "rgba(255,255,255,0.7)", fontFamily: "'Lato'", fontSize: "12px", fontWeight: 700, cursor: "pointer" }}>Apply</button>
            </div>
            {couponError && <p style={{ fontFamily: "'Lato'", fontSize: "11px", color: "#e8614d", marginTop: "6px" }}>{couponError}</p>}
            {couponSuccess && <p style={{ fontFamily: "'Lato'", fontSize: "11px", color: "#2a9d8f", marginTop: "6px" }}>{couponSuccess}</p>}
          </div>
        )}
      </div>
    </div>
